...
(++) ::
  ...
  -> List a
  -> List a
(++) xs ys = foldRight (\x zs -> x :. zs) ys xs
...
flatten ::
  List (List a)
  -> List a
flatten = foldRight (\xs ys -> foldRight (\x zs -> x :. zs) ys xs) Nil
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
seqOptional = foldRight (\ox acc -> bindOptional (\x -> mapOptional (\s -> x :. s) acc) ox) (Full Nil)
...
notReverse ::
  List a
  -> List a
notReverse = reverse -- not sure what this Q is getting at???
